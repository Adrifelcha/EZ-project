...
source("../scripts/HDDM_runFullSeed.R")
library(foreach)
library(doParallel)
...
modelFile <- matrix(c(rep("./EZHBDDM.bug" ,3), 
                      rep(c("./EZHBDDM_BetaDrift.bug","./EZHBDDM_BetaNondt.bug","./EZHBDDM_BetaBound.bug"),2)), 
                    byrow = TRUE, ncol = 3)
...
settings <- list("fromPrior" = fromPrior,  # This logical variable should be defined before running this script
                 ...
                 "n.chains" = 2)
settings <- c(settings,
              list("nCells" = prod(length(settings$participant_levels),length(settings$trial_levels))*(1+3*2)))
#################| Specific JAGS objects
jagsInits <- list()
    for(i in settings$participant_levels){
      jagsInits <- c(jagsInits, list(default_inits(settings$n.chains, i)))
    }
settings <- c(settings, list("jagsParameters" = list(jagsParameters, c(jagsParameters, "betaweight"), 
                                                     c(jagsParameters, "betaweight")),
                             "modelFile" = modelFile,
                             "priors" = list(default_priors(Show=FALSE, "hierarchical"), default_priors(Show=FALSE, "ttest"),
                                            default_priors(Show=FALSE, "metaregression")),
                             "jagsData" = list(data_toJAGS("hierarchical"), data_toJAGS("ttest"), data_toJAGS("metaregression")),
                             "jagsInits" = jagsInits))
...
for(model in settings$design_levels){
    for(crit in settings$criterion_levels){
        write_JAGSmodel(settings$priors[[3]], model, crit, settings$modelFile[model,crit])
    }
}
...
output <- foreach(i = 1:settings$nDatasets, 
                  ...
                  .combine = 'rbind'
                    ...
                    Z <- HDDM_runFullSeed(seed = i, settings)
                  }
