...
Data from [Vandekerckhove, Panis and Wagemans (2007)](https://link.springer.com/article/10.3758/BF03193960), experiment 1, participant N.H.:
...
```{r}
data <- read.csv("./shapePerception_ezdiff.csv")
head(data)
```
...
We also include a nonlinear regression on the drift rate $\delta$ using four dummy variables to differentiate across all five conditions: 

- $X$ indicates whether the change was qualitative $X = 0$ or quantitative $X = 1$
- $Y$ indicates whether there was a change on convexity $Y = 0$ or concavity $Y = 1$
- $Z$ indicates whether the condition included a change $Z = 0$, identifying the special condition where there are NO changes $Z = 1$.

\begin{align*}
\delta_\mbox{pred,k} &= \mu_\delta+Z_k(\gamma_1X_k+\gamma_2Y_k+\gamma_3X_kY_k)+\gamma_4Z_k\\
\delta_k &\sim \mbox{Normal}(\delta_\mbox{pred,k},\sigma_\delta)
\end{align*}
...
model <- write("
model {
          mu ~ dnorm(0,1)   # Baseline
          for(i in 1:4){
              gamma[i] ~ dnorm(0,1)   
          }      
    
    gamma[1] ~ dnorm(0,1)   # Qual vs Quant
    gamma[2] ~ dnorm(0,1)   # Concave vs Convex
    gamma[3] ~ dnorm(0,1)   # Interaction
    gamma[4] ~ dnorm(0,1)   # No-change condition

    Mdrift[1] = mu
    Mdrift[2] = mu + gamma[1]
    Mdrift[3] = mu + gamma[2]
    Mdrift[4] = mu + gamma[1] + gamma[2] + gamma[3]
    Mdrift[5] = mu + gamma[4]

    Pdrift ~ dgamma(2,1)
    SDdrift = pow(Pdrift, -0.5)
        
    for (k in 1:length(N)) {
        # Person-by-condition parameters for DM parameters
        alpha[k] ~ dgamma(2,1)
        tau[k]   ~ dexp(1)
        drift[k] ~ dnorm(Mdrift[cond[k]],Pdrift)

        # Forward equations from EZ Diffusion
        y[k]   = -alpha[k] * drift[k]
        Pc[k]  = ilogit(-y[k])
        PRT[k] = 2 * pow(drift[k], 3) / alpha[k] * pow(exp(y[k]) + 1, 2) / (2 * y[k] * exp(y[k]) - exp(2 * y[k]) + 1) #precision
        MDT[k] = (alpha[k] / (2 * drift[k])) * (1 - exp(y[k])) / (1 + exp(y[k]))
        MRT[k] = MDT[k] + tau[k]

        # Loss functions using MRT, PRT, and Pc
        meanrt[k] ~ dnorm(MRT[k], PRT[k] * sumacc[k])
        varrt[k]  ~ dnorm(1/PRT[k], 0.5 * sumacc[k] * PRT[k] * PRT[k])
        sumacc[k] ~ dbin(Pc[k], N[k])
    }
