...
```{r}
simH <- HDDM_runSims(nParticipants = 50, nTrials = 300, nDatasets = 1, modelType = "hierarchical", forceSim = TRUE)
```
...
## 300 trials - 30 participants - 100 iter

```{r, results='hide',message=FALSE, eval=FALSE}
simStudyH1 <- HDDM_runSims(nParticipants = 30, nTrials = 300, nDatasets = 100, modelType = "hierarchical", Show=TRUE)
```

```{r, results='hide',message=FALSE, eval=FALSE}
plot_recovery(simStudyH1)
```

## 30 trials - 300 participants - 100 iter

```{r, results='hide',message=FALSE, eval=FALSE}
simStudyH2 <- HDDM_runSims(nParticipants = 300, nTrials = 30, nDatasets = 100, modelType = "hierarchical", Show=TRUE)
```

```{r, results='hide',message=FALSE, eval=FALSE}
plot_recovery(simStudyH2)
```

# Metaregression model

```{r}
simM <- HDDM_runSims(nParticipants = 50, nTrials = 300, nDatasets = 1, modelType = "metaregression", forceSim = TRUE)
```
...
```{r}
simStudyM1 <- HDDM_runSims(nParticipants = 30, nTrials = 300, nDatasets = 100, modelType = "metaregression", Show=TRUE)
```

```{r, results='hide',message=FALSE}
plot_recovery(simStudyM1)
```

## 30 trials - 300 participants - 100 iter

```{r, results='hide',message=FALSE, eval=FALSE}
simStudyM2 <- HDDM_runSims(nParticipants = 300, nTrials = 30, nDatasets = 100, modelType = "metaregression",  Show=TRUE)
```


```{r, results='hide',message=FALSE, eval=FALSE}
plot_recovery(simStudyM2)
```

# T-test model

```{r}
simT <- HDDM_runSims(nParticipants = 100, nTrials = 200, nDatasets = 1, modelType = "ttest", forceSim = TRUE)
```
...
```{r, results='hide',message=FALSE}
simStudyT1 <- HDDM_runSims(nParticipants = 30, nTrials = 300, nDatasets = 100, modelType = "ttest", Show=TRUE)
```

```{r, results='hide',message=FALSE}
plot_recovery(simStudyT1)
```

## 30 trials - 300 participants - 100 iter

```{r, results='hide',message=FALSE, eval=FALSE}
simStudyT2 <- HDDM_runSims(nParticipants = 300, nTrials = 30, nDatasets = 100, modelType = "ttest", Show=TRUE)
```

```{r, results='hide',message=FALSE, eval=FALSE}
plot_recovery(simStudyT2)
```
